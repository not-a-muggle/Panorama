...
router.post('/share', async (req: express.Request, res: express.Response) => {
    ...
    */
    ...
    if (!shareeId || !sharerId || !imageIds) {
        console.log("POST /share - Insufficient information in the request");
        if (!shareeId) {
    ...
    try {
        const result: boolean = await serviceInstance.shareImages(sharerId, shareeId, imageIds);
        if (!result) {
            console.log(`/POST share - error while shareing images`);
            res.sendStatus(500);
            return;
        }
        res.sendStatus(200);
        console.log('/POST share - Share images successful');
        return;
    } catch (ex) {
        console.log(`/POST share - error while shareing images \n${ex}`);
        res.sendStatus(500);
